...
class block_ranking extends block_base {
    ...
    public function get_content() {
		...
		$rankingtitle = '<h4 class="title">'. format_text("Leader Board", FORMAT_HTML, array('filter' => true)) .'</h4>';
        //$this->content->text = $rankingtitle.$individualranking.$rankingstables;
		  $this->content->text = $rankingtitle.$rankingstables;
        ...
        if (has_capability('moodle/site:accessallgroups', $context)) {
			$this->content->footer .= html_writer::tag('p',
                                          html_writer::link(
                                              new moodle_url(
                                                  '/blocks/ranking/editranking.php',
                                                  array('courseid' => $this->page->course->id)
                                              ),
                                              get_string('edit_ranking', 'block_ranking'),
                                              array('class' => 'ranklink')
                                          )
                                    );
